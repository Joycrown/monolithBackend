...
class BlockDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Block
        fields = ['id','avatar','cover', 'name', 'desc', 'about', 'category', 'block_type', 'subscribers', 'moderators', 'creator', 'created', 'subscriber_count', 'is_deleted', 'slug', 'numPosts']   
...
class RuleSerializer(serializers.ModelSerializer):
    class Meta:
        model = Rule
        fields = ('id', 'block', 'title', 'text', 'last_modified')


class LinkSerializer(serializers.ModelSerializer):
    class Meta:
        model = Link
        fields = ('id', 'block', 'image', 'title', 'url', 'last_modified')
...
class CommentSerializer_detailed(serializers.ModelSerializer):
    class Meta:
        model = Comment
        fields = ['id','post','author', 'parent_comment', 'report', 'saved', 'created', 'text', 'is_deleted', 'saved_count', 'report_count', 'share_count', 'voters', 'votes', 'saved_count', 'report_count', 'child_comments', 'created_time_ago']
        depth = 1
...
class PostSerializer_detailed(serializers.ModelSerializer):
    class Meta:
        model = Post
        fields = ['id','title', 'block', 'attachment', 'created', 'link', 'text', 'post_type', 'parent', 'author', 'report', 'saved', 'share_count', 'saved_count', 'report_count', 'is_deleted', 'is_repost', 'is_reviewed', 'voters', 'votes', 'reposts', 'comments', 'slug', 'created_time_ago']
        depth = 1
