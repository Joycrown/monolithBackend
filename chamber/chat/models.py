...
def chamber_to(instance, filename):
    return "avatars/{0}/{1}".format(instance.room_name, filename)


def chamber_for(instance, filename):
    return "covers/{0}/{1}".format(instance.room_name, filename)


def file_to(instance, filename):
    return "chambers/{0}/{1}/{2}".format(instance.room.room_name, instance.from_user.username, filename)


def add_chamber_util(instance):
    now = date.today()
...
class Chamber(models.Model):
    avatar = models.ImageField(
        upload_to=chamber_to, blank=True, null=True, max_length=1000000
    )
    cover = models.ImageField(
        upload_to=chamber_for, blank=True, null=True, max_length=1000000
    )
    ...
    chamber_type = models.CharField(max_length=100, null=True, blank=True)
...
class Chat(models.Model):
    room = models.ForeignKey(Chamber, on_delete=models.CASCADE, null=True)
